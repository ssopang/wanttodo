...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var NTab = document.getElementById('N-tab');
    
    if (NTab) {
    	NTab.classList.add('active');  // 'active' 클래스 추가
    	NTab.setAttribute('aria-selected', 'true');  // aria-selected 속성 설정
    }

   
    var NTabContent = document.getElementById('N');
    if (NTabContent) {
    	NTabContent.classList.add('active', 'show');  // 'active', 'show' 클래스 추가
    }

    
    NTab.addEventListener('click', function(event) {
        // 기본 동작 막기 (href를 통한 이동을 방지)
        event.preventDefault();
        
        // 페이지 1번으로 강제 이동
        //window.location.href = '${contextPath}/admin/goods/NmodgoodsList.do?goods_status=N&pageNum=1';
    });
});
</script>
        ...
        if (searchQuery === '') {
            ...
            <ul class="nav nav-tabs justify-content-center" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link title1"  href="${contextPath}/admin/goods/modgoodsListBean.do?goods_category=원두" role="tab" aria-controls="beans" aria-selected="true">원두</a>&nbsp;&nbsp;&nbsp;&nbsp;
                </li>
                <li class="nav-item" role="presentation">
                ...
                <li class="nav-item" role="presentation">
                   ...
                   <li class="nav-item" role="presentation">
                    <a class="nav-link title1"id="N-tab" href="${contextPath}/admin/goods/NmodgoodsList.do?goods_category=원두">미등록 상품</a>
                </li>
            </ul>
			<br>
            <!-- 상위 탭 내용 -->
...
<!-- 원두 탭 -->
<div class="tab-pane fade show" id="N" role="tabpanel" aria-labelledby="N-tab">
...
<ul class="pagination">
    <c:forEach var="page" begin="1" end="10" step="1">
        ...
        <li class="page-item">
            <a class="page-link" href="${contextPath}/admin/goods/NmodgoodsList.do?goods_category=원두&section=${section-1}&pageNum=${(section-1)*10 + 1}">&laquo;</a>
       </li>
        </c:if>

        <!-- 현재 페이지 번호 -->
         <li class="page-item">
        <a class="page-link" href="${contextPath}/admin/goods/NmodgoodsList.do?goods_category=원두&section=${section}&pageNum=${(section-1)*10 + page}">${(section-1)*10 + page}</a>
 		</li>
        <!-- 다음 페이지 링크 -->
        <c:if test="${page == 10}">
        <li class="page-item">
            <a class="page-link" href="${contextPath}/admin/goods/NmodgoodsList.do?goods_category=원두&section=${section+1}&pageNum=${section*10 + 1}">&raquo;</a>
        </li>
        </c:if>
    </c:forEach>
    </ul>
