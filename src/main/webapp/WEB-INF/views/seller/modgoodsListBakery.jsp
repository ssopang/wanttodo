...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var bakeryTab = document.getElementById('bakery-tab');
    
    if (bakeryTab) {
    	bakeryTab.classList.add('active');  // 'active' 클래스 추가
    	bakeryTab.setAttribute('aria-selected', 'true');  // aria-selected 속성 설정
    }

    // 원두 탭의 콘텐츠를 활성화
    var bakeryTabContent = document.getElementById('bakery');
    if (bakeryTabContent) {
    	bakeryTabContent.classList.add('active', 'show');  // 'active', 'show' 클래스 추가
    }

    // 원두 탭 클릭 시 페이지 1번으로 이동
    bakeryTab.addEventListener('click', function(event) {
        // 기본 동작 막기 (href를 통한 이동을 방지)
        event.preventDefault();
        
        // 페이지 1번으로 강제 이동
        window.location.href = '${contextPath}/seller/modgoodsListBakery.do?goods_category=베이커리&pageNum=1&mem_id=${mem_id}';
    });
});
...
<div class="top-right-area">
    <form action="${contextPath}/seller/modsearchGoods.do" method="get" class="search-bar"onsubmit="submitSearchForm(event)">
...
<ul class="nav nav-tabs justify-content-center" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link title1" href="${contextPath}/seller/modgoodsListBean.do?goods_category=원두&mem_id=${mem_id}">원두</a>&nbsp;&nbsp;&nbsp;&nbsp;
                </li>
                <li class="nav-item" role="presentation">
                   <a class="nav-link title1" href="${contextPath}/seller/modgoodsListBakery.do?goods_category=베이커리&mem_id=${mem_id}">베이커리</a>&nbsp;&nbsp;&nbsp;&nbsp;
                </li>
                <li class="nav-item" role="presentation">
                   
                   <a class="nav-link title1" id="coffee-tab"href="${contextPath}/seller/modgoodsListTool.do?goods_category=커피용품&mem_id=${mem_id}" role="tab" aria-controls="coffee" aria-selected="false">커피 용품</a>&nbsp;&nbsp;&nbsp;&nbsp;
                </li>
                           <li class="nav-item" role="presentation">
    ...
    <div class="main_book mb-5">
        <div class="row">
            ...
            <c:forEach var="goods" items="${goodsList}">
                <!-- 카테고리가 '베이커리'인 상품만 출력 -->
                <c:if test="${goods.goods_category == '베이커리'}">
...
<ul class="pagination">
    <c:forEach var="page" begin="1" end="10" step="1">
        ...
        <li class="page-item">
            <a class="page-link" href="${contextPath}/seller/modgoodsListBakery.do?goods_category=베이커리&section=${section-1}&pageNum=${(section-1)*10 + 1}&mem_id=${mem_id}">&laquo;</a>
       </li>
        </c:if>

        <!-- 현재 페이지 번호 -->
         <li class="page-item">
        <a class="page-link" href="${contextPath}/seller/modgoodsListBakery.do?goods_category=베이커리&section=${section}&pageNum=${(section-1)*10 + page}&mem_id=${mem_id}">${(section-1)*10 + page}</a>
 		</li>
        <!-- 다음 페이지 링크 -->
        <c:if test="${page == 10}">
        <li class="page-item">
            <a class="page-link" href="${contextPath}/seller/modgoodsListBakery.do?goods_category=베이커리&section=${section+1}&pageNum=${section*10 + 1}&mem_id=${mem_id}">&raquo;</a>
        </li>
        </c:if>
    </c:forEach>
    </ul>
